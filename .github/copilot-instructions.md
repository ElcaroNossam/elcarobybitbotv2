...
## ✅ ОБЯЗАТЕЛЬНАЯ ВЕРИФИКАЦИЯ ПОСЛЕ КАЖДОГО ЗАПРОСА

**После КАЖДОГО выполненного запроса (fix, feat, refactor) ОБЯЗАТЕЛЬНО провести мини-аудит:**

1. **Проверить все аналогичные места** — если исправляется баг в одной стратегии, проверить ВСЕ 6 стратегий
2. **Проверить edge cases** — что будет если значение = 0, None, пустая строка?
3. **Проверить ресурсы** — все адаптеры/соединения закрываются? try/finally присутствует?
4. **Проверить multitenancy** — exchange параметр передаётся? account_type корректный?
5. **Проверить кеш** — ключи кеша содержат все необходимые измерения (user_id, symbol, account_type)?
6. **Grep по паттерну** — найти все места в коде с аналогичным паттерном и убедиться что они тоже корректны

**Чек-лист для торговых изменений:**
- [ ] TP guard: `if tp_pct else None` — TP=0 не ставит TP на цену входа
- [ ] SL guard: SL всегда устанавливается (не может быть None для торговой позиции)
- [ ] HLAdapter: `initialize()` вызывается ПЕРЕД любым методом, `close()` в `finally`
- [ ] Leverage: никогда не инфлейтить выше пользовательской настройки
- [ ] order_type: читается из `params.get("order_type")`, не из `strat_settings`
- [ ] reduce_only: передаётся при закрытии позиций (PTP, manual close)
- [ ] exchange field: передаётся во все DB операции (add_active_position, log_exit, get_trade_stats)

**Чек-лист для API/Security изменений:**
- [ ] Rate limiting на все auth endpoints
- [ ] `secrets.compare_digest()` для сравнения кодов/токенов
- [ ] JWT required для всех финансовых endpoints
- [ ] IDOR protection: user может видеть только СВОИ данные
- [ ] Credentials никогда в URL query params — только в body

**❌ ЗАПРЕЩЕНО:**
- Завершать запрос без проверки аналогичных мест
- Игнорировать edge cases (0, None, пустые значения)
- Оставлять незакрытые адаптеры/соединения
