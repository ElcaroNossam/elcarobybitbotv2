...
<b>Решение:</b> Уменьшите плечо в настройках стратегии.''',
    ...
    # ═══════════════════════════════════════════════════════════
    # ЕЖЕДНЕВНЫЕ УВЕДОМЛЕНИЯ ОБ ОШИБКАХ (раз в день на тип)
    # ═══════════════════════════════════════════════════════════
    
    # Нулевой баланс (раз в день)
    'daily_zero_balance':          '''⚠️ <b>ВНИМАНИЕ: БАЛАНС</b>

💰 На вашем <b>{account_type}</b> аккаунте <b>$0</b>.

📊 <b>Пропущено сигналов сегодня:</b> {missed_count}

<b>🧠 Чтобы возобновить торговлю:</b>
• Пополните {account_type} аккаунт
• Или переключитесь на другой аккаунт

<i>Это ежедневная сводка. Сигналы ждут.</i>

👉 /balance — <b>Проверить баланс</b>''',

    # Невалидные API ключи (раз в день)
    'daily_api_keys_invalid':      '''🔑 <b>ПРОБЛЕМА С API КЛЮЧАМИ</b>

⚠️ Ваши <b>{account_type}</b> API ключи недействительны.

📊 <b>Пропущено сигналов:</b> {missed_count}

<b>🔧 Исправить:</b>
1. Bybit → API Management
2. Создайте новые ключи
3. Обновите в /api_settings

<i>Без ключей бот не может торговать.</i>''',

    # Ошибка подключения (раз в день)
    'daily_connection_error':      '''🌐 <b>ОШИБКА ПОДКЛЮЧЕНИЯ</b>

⚠️ Не удаётся подключиться к <b>{exchange}</b> ({account_type}).
...
<i>Бот будет пробовать снова автоматически.</i>''',

    # Маржа исчерпана
    'daily_margin_exhausted':      '''📊 <b>ВНИМАНИЕ: МАРЖА</b>

💰 Вся маржа <b>{account_type}</b> занята.

📊 <b>Открытых позиций:</b> {open_count}
📊 <b>Пропущено сигналов:</b> {missed_count}
...
<i>Эффективность капитала - ключ к прибыли.</i>

👉 /positions — <b>Управление позициями</b>''',
...
• Закройте другие позиции''',
    ...
    'be_settings_desc':              '_Перенос SL на уровень входа при достижении прибыли_',
    'be_enabled_label':              '🔒 Безубыток',
    'be_trigger_label':              '🎯 Триггер БУ %',
    'prompt_be_trigger':             'Введите триггер безубытка % (например, 1.0):',
    'prompt_long_be_trigger':        '📈 LONG Триггер БУ %\n\nВведите % прибыли для переноса SL на вход:',
    'prompt_short_be_trigger':       '📉 SHORT Триггер БУ %\n\nВведите % прибыли для переноса SL на вход:',
    'param_be_trigger':              '🎯 Триггер БУ %',
    'be_moved_to_entry':             '🔒 {symbol}: SL перенесён в безубыток @ {entry}',
    'be_status_enabled':             '✅ БУ: {trigger}%',
    'be_status_disabled':            '❌ БУ: Выкл',
    
    # Partial Take Profit settings UI (срез маржи)
    'partial_tp_label':              '✂️ Срез маржи',
    'partial_tp_status_enabled':     '✅ Срез маржи включён',
    'partial_tp_status_disabled':    '❌ Срез маржи выкл',
    'partial_tp_step1_menu':         '✂️ *Срез маржи - Шаг 1*\n\nЗакрыть {close}% позиции при +{trigger}% прибыли\n\n_Выберите параметр:_',
    'partial_tp_step2_menu':         '✂️ *Срез маржи - Шаг 2*\n\nЗакрыть {close}% позиции при +{trigger}% прибыли\n\n_Выберите параметр:_',
    'trigger_pct':                   'Триггер',
    'close_pct':                     'Закрыть',
    'prompt_long_ptp_1_trigger':     '📈 LONG Шаг 1: Триггер %\n\nВведите % прибыли для закрытия части:',
...
✅ Все обновления''',
    ...
    'premium_3m': '💎 3 Месяца — {price} ELC (−10%)',
    'premium_6m': '💎 6 Месяцев — {price} ELC (−20%)',
    'premium_12m': '💎 12 Месяцев — {price} ELC (−30%)',
...
Используйте ELC токены.''',
    ...
    'payment_ton_not_configured': '❌ TON платежи устарели. Используйте ELC токены.',
    'payment_session_expired': '❌ Сессия оплаты истекла. Начните заново.',
    'payment_elc_insufficient': '''❌ Недостаточно ELC.
...
Пополните кошелёк для продолжения.''',
    ...
    'wallet_balance': '''💰 *Ваш ELC Кошелёк*

◈ Баланс: *{balance} ELC*
📈 В стейкинге: *{staked} ELC*
🎁 Ожидающие награды: *{rewards} ELC*
...
📍 1 ELC = 1 USDT''',
    ...
    'wallet_deposit_desc': '''Отправьте ELC токены на адрес кошелька:

`{address}`

💡 *Демо режим:* Нажмите ниже для получения тестовых токенов.''',
    'wallet_deposit_demo': '🎁 Получить 100 ELC (Демо)',
    'wallet_deposit_success': '✅ Пополнено {amount} ELC успешно!',
    ...
    'wallet_stake_desc': '''Стейкайте ELC токены и получайте *12% годовых*!
...
Ежедневные награды • Мгновенный вывод''',
    'wallet_stake_success': '✅ {amount} ELC успешно застейкано!',
    'wallet_unstake_success': '✅ Снято {amount} ELC + {rewards} ELC наград!',
...
''',
    ...
    # =====================================================
    'error_insufficient_balance': '💰 Недостаточно средств на аккаунте для открытия позиции. Пополните баланс или уменьшите размер позиции.',
    'error_order_too_small': '📉 Размер ордера слишком мал (минимум $5). Увеличьте Entry% или пополните баланс.',
    'error_api_key_expired': '🔑 API ключ истёк или недействителен. Обновите API ключи в настройках.',
    'error_api_key_missing': '🔑 API ключи не настроены. Добавьте ключи Bybit в меню 🔗 API Keys.',
    'error_rate_limit': '⏳ Слишком много запросов. Подождите минуту и попробуйте снова.',
    'error_position_not_found': '📊 Позиция не найдена или уже закрыта.',
    'error_leverage_error': '⚙️ Ошибка установки плеча. Попробуйте установить плечо вручную в терминале биржи.',
    'error_network_error': '🌐 Проблема с сетью. Попробуйте позже.',
    'error_sl_tp_invalid': '⚠️ Невозможно установить SL/TP: цена слишком близко к текущей. SL/TP будут обновлены при следующем цикле.',
    'error_equity_zero': '💰 На аккаунте нулевой баланс. Пополните Demo или Real аккаунт для торговли.',
    
    # =====================================================
    # HARDCODED STRINGS FIX (Jan 27, 2026)
    # =====================================================
    'terminal_button': '💻 Терминал',
    'exchange_mode_activated_bybit': '🟠 *Режим Bybit активирован*',
    'exchange_mode_activated_hl': '🔷 *Режим HyperLiquid активирован*',
    'error_processing_request': '⚠️ Ошибка обработки запроса',
    'unauthorized_admin': '❌ Не авторизовано. Эта команда только для владельца.',
    'error_loading_dashboard': '❌ Ошибка загрузки дашборда.',
    'unauthorized': '❌ Не авторизовано.',
    'processing_blockchain': '⏳ Обрабатываем транзакцию в блокчейне...',
    'verifying_payment': '⏳ Проверяем платёж в TON блокчейне...',
