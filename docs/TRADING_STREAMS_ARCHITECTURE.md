# 🔀 Trading Streams Architecture
# ElCaro Trading Platform - Полная карта потоков
# ================================================
# Версия: 1.0 | 20 января 2026
...
└─────────────────────────────────────────────────────────────────────────────────┘
           ...
           ▼                                                         ▼
┌─────────────────────────┐                             ┌─────────────────────────┐
│      BYBIT (CEX)        │                             │   HYPERLIQUID (DEX)     │
│  exchange = 'bybit'     │                             │  exchange = 'hyperliquid'│
└─────────────────────────┘                             └─────────────────────────┘
           │                                                         │
     ┌─────┴─────┐                                             ┌─────┴─────┐
     │           │                                             │           │
...
│                           5 СТРАТЕГИЙ                                            │
│  ┌─────────┐ ┌─────────────┐ ┌─────────┐ ┌─────────┐ ┌─────────────┐            │
│  │   OI    │ │ Scryptomera │ │ Scalper │ │ ElCaro  │ │  Fibonacci  │            │
│  └─────────┘ └─────────────┘ └─────────┘ └─────────┘ └─────────────┘            │
└─────────────────────────────────────────────────────────────────────────────────┘
                                     │
                    ┌────────────────┼────────────────┐
...
### Формула:
```
ПОТОКИ = Биржи × Аккаунты × Стратегии × Направления
       = 2 × 2 × 5 × 3
       = 60 потенциальных потоков на юзера
```

### Реальность:
- Большинство юзеров используют **1 биржу** (Bybit ИЛИ HyperLiquid)
- Большинство используют **1 аккаунт** (Demo ИЛИ Real)
- Большинство используют **1-2 стратегии**
- Направление обычно `'all'` (не разделяют Long/Short)

**Типичный юзер: 1 × 1 × 2 × 1 = 2 активных потока**
...
└──────┬───────┘
       ...
       ▼
┌──────────────────────────────────────────────────────────────────┐
│  1. Получить exchange_type юзера                                  │
│     exchange = db.get_exchange_type(uid)  # 'bybit'              │
└──────────────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────────────┐
│  2. Получить trading_mode юзера                                   │
│     mode = db.get_trading_mode(uid)  # 'demo' / 'real' / 'both'  │
└──────────────────────────────────────────────────────────────────┘
       │
       ▼
┌──────────────────────────────────────────────────────────────────┐
│  3. Определить target accounts                                    │
│     if mode == 'both':                                           │
│         targets = ['demo', 'real']  # или ['testnet', 'mainnet'] │
│     else:                                                         │
│         targets = [mode]                                          │
└──────────────────────────────────────────────────────────────────┘
       │
...
# 'both' → 'demo' (Bybit) или 'testnet' (HL)
acc = _normalize_both_account_type('both', exchange='bybit')  # → 'demo'
acc = _normalize_both_account_type('both', exchange='hyperliquid')  # → 'testnet'
```
...
*Версия: 1.1 - добавлена оптимизированная fallback логика с 'default'*
