...
struct SignalsView: View {
    @ObservedObject private var signalsService = SignalsService.shared
    ...
    var body: some View {
        NavigationView {
            ...
            .onChange(of: selectedStrategy) { oldValue, newValue in
                Task {
                    signalsService.selectedStrategy = newValue
                    await signalsService.fetchSignals(strategy: newValue)
                    await signalsService.fetchStats(strategy: newValue)
                }
            }
        }
    }
