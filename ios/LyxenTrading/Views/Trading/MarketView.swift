            ...
            .toolbar {
                ToolbarItem(placement: .topBarTrailing) {
                    Button(action: { webSocket.connect() }) {
                        Circle()
                            .fill(webSocket.isConnected ? Color.lyxenGreen : Color.lyxenRed)
                ...
                ForEach(SortOption.allCases, id: \.self) { option in
                    Button(action: { sortBy = option }) {
                        Text(option.rawValue)
                            ...
                            .padding(.vertical, 6)
                            .background(sortBy == option ? Color.lyxenPrimary : Color.lyxenCard)
                            .foregroundColor(sortBy == option ? .white : .lyxenTextSecondary)
                            .cornerRadius(16)
                    }
                }
            }
    ...
    var body: some View {
        HStack {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                ...
                .font(.caption.weight(.medium))
                .foregroundColor(ticker.priceChangePercent >= 0 ? .lyxenGreen : .lyxenRed)
            }
...
struct SymbolDetailView: View {
            ...
            HStack(spacing: 16) {
                ForEach(["1H", "4H", "1D", "1W"], id: \.self) { tf in
                    Text(tf)
                        ...
                        .padding(.horizontal, 12)
                        .padding(.vertical, 6)
                        .background(tf == "1D" ? Color.lyxenPrimary : Color.clear)
                        .foregroundColor(tf == "1D" ? .white : .lyxenTextSecondary)
                        .cornerRadius(8)
                }
            }
    ...
    private var statsGrid: some View {
        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], spacing: 12) {
            StatBox(title: "24h High", value: "$67,890.00", color: .lyxenGreen)
            StatBox(title: "24h Low", value: "$63,210.00", color: .lyxenRed)
            StatBox(title: "24h Volume", value: "$1.2B", color: .lyxenBlue)
            StatBox(title: "Open Interest", value: "$890M", color: .lyxenYellow)
        }
    }
}
